...
class Gamble(commands.Cog):
    ...
    async def coinflip(self, ctx, *args):
        ...
        if len(args) > 2:
            return await ctx.send(
                f"❗Invalid arguments **{ctx.author.name}**|Please include the betting amount and the choice[optional]. "
            )
        if args:
            for amt in args:
                if amt.isdigit():
        ...
        if bet < 1:
            await ctx.send(f"**{ctx.author.name}** you can't bet that...")
        else:
            if await functions.check_balance(ctx.author.id) < bet:
                ...
                message = await ctx.send(
                    f"**{ctx.author.name}** spent {bet} <:chigs:973011363041517598> and chose {choice}\nThe coin spins... <a:cf:939070721504706572>"
                )
                ...
                if choice == win_condition:
                    ...
                    await message.edit(
                        content=f"**{ctx.author.name}** spent {bet} <:chigs:973011363041517598> and chose {choice}\nThe coin spins...and you won {win_amount} <:chigs:973011363041517598> !! "
                    )
    ...
    async def rolldice(self, ctx, bet_amount=1):
        ...
        if bet_amount < 1:
            ...
            message = await ctx.send(
                f"**{ctx.author.name}** spent {bet_amount} <:chigs:973011363041517598> and chose {choice}\nThe die rolls... "
            )
            await asyncio.sleep(2)
            if choice == win_condition:
                if choice in (1, 2, 3):
                ...
                await message.edit(
                    content=f"**{ctx.author.name}** spent {bet_amount} <:chigs:973011363041517598> and chose {choice}\nCongoo <a:Yeeey:876323873069596712> {win_condition} turns up...and you won {win_amount} <:chigs:973011363041517598> !! "
                )
