...
async def check_balance(uid):
    ...
    prefetcher_file = os.path.join(
        os.path.dirname(__file__), "./prefetch_data/cash_cache.json"
    )
...
async def cash_prefetcher():
    ...
    prefetcher_cash_file = os.path.join(
        os.path.dirname(__file__), "./prefetch_data/cash_cache.json"
    )
    db = con.create_conn()
    cursor = db.cursor()
...
async def cash_postsyncer(uid_list):
    ...
    """
    postsyncer_cash_file = os.path.join(
        os.path.dirname(__file__), "./prefetch_data/cash_cache.json"
    )
    ...
    for uid in uid_list:
        cursor.execute(
            f'UPDATE CASH SET CASH = {data.get(str(uid))} WHERE id = "{uid}"'
        )
...
async def inventory_prefetcher():
    ...
    prefetcher_inventory_file = os.path.join(
...
async def inventory_postsyncer(uid_list):
    ...
    Syncs the data from the json file to the database.
    """
    postsyncer_inventory_file = os.path.join(
        os.path.dirname(__file__), "./prefetch_data/inventory_cache.json"
    )
