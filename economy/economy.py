...
class Bot(commands.Cog):
    ...
    async def cash(self, ctx, user: discord.Member = None):
        ...
        await ctx.reply(
            f"<:chigs:973011363041517598> **{user.name}** currently has **{cash:,}** chigs."
        )
    ...
    async def sendchigs(self, ctx, cash, user: discord.Member = None):
        ...
        if cash.isdigit():
            cash = int(cash)
            if cash is None or user is None:
                await ctx.send(
                    f"❗Invalid arguments **{ctx.author.name}** |Please include cash and the user. "
                )
            elif cash < 0:
                await ctx.send(
                    f"**{ctx.author.name}** Well are you basically trying to rob **{user.name}** _tch tch_..."
                )
                ...
                if await functions.check_balance(ctx.author.id) < cash:
                    await ctx.send(
                        f"**{ctx.author.name}** you cannot send that much chigs... "
                    )
                    ...
                    await ctx.send(
                        f"**{ctx.author.name}** sent {cash} chigs <:chigs:973011363041517598> to **{user.name}**!"
                    )
    ...
    async def beg(self, ctx):
        ...
        if prob == "yes":
            ...
            phrases = [
                f"Oh poor! I have only {beg_amount} <:chigs:973011363041517598> to give.",
                f"You got {beg_amount} <:chigs:973011363041517598> from the strangers passing by.",
                f"What a pity! Here {beg_amount} <:chigs:973011363041517598> ...Hope it would help you a bit.",
                f"Here **{ctx.author.name}**, take {beg_amount} <:chigs:973011363041517598> ...have a great day! UwU",
            ]
    ...
    async def buy(self, ctx, item, amount=1):
        ...
        if await functions.check_balance(ctx.author.id) < price:
            ...
            await ctx.send(
                f"**{ctx.author.name}** you don't even have that much chigs... "
            )
            ...
            await ctx.send(
                f"**{ctx.author.name}** || purchased {amount} {item} for {price} <:chigs:973011363041517598>!"
            )
